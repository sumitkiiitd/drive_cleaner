...
month/year headers). Grouping uses each photo's actual capture time (EXIF
`imageMediaMetadata.time`, or `videoMediaMetadata.time` for videos) rather
than when it was uploaded to Drive, so imported photos land in the right
